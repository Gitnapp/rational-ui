...
- 折叠态图标轨（`48px`）的入口高亮固定 `size-8` 并水平居中。触屏命中区只在展开态用
  36px 可视高度加透明命中边缘，**不得把 44px 高度带进折叠态**，否则高亮退化成长方形。
...
- 卡片宽度随容器调整，同一网格内等高；底部操作对齐，不用固定像素宽度撑破手机视窗。
- CardHeader 与 CardContent 间距统一，状态放在标题行；InfoLabel/InfoHint 使用透明背景、小尺寸和统一居中对齐，禁止叠加 margin 撑高标题空白。
- 父子异步内容共用 LoadingScope，只出现一个 spinner。已有内容的后台刷新保留当前内容；不使用多层中间态或可见“加载中”文字。
- 操作上下文清楚时只用动词，保持默认箭头光标。低频管理动作集中到菜单，选项需有清楚的组标题。
- 数值默认只读，点击才编辑；来源状态可区分 AI 与用户设定。手动来源悬停或键盘聚焦时显示 Undo，撤销事件交给宿主处理；业务持久化不进入组件库。
- 日期优先一键常用范围，最多四个预设加一个自定义入口。自定义在同一日历点两天即生效，无额外确认按钮；不用两组原生日期输入或用途不明的滑条。
- 目录为高亮状态预留宽度，禁止换行；高亮使用字重和字号，不加灰底或侧边黑线。空间不足收为圆点，悬停/聚焦展开；手机使用当前章节入口和可滚动目录面板。
- 面包屑反映真实路由，父级可点击，当前页使用 aria-current。移动端也必须可见，不能隐藏唯一返回入口。
- 同类实体共用相同卡片与字段，缺失值显示 —；不因为数据源缺失而替换成其他字段或整块隐藏。
- 情景、过滤和局部编辑保留阅读位置；只有显式页面导航才重置滚动。
- 通用说明使用 *；引用使用有序编号、固定标记列。图表坐标、图例和标识留出独立空间。

以上规则由共享基础组件与 patterns.css 实现。应用保留数据源、计算、路由和保存职责。

## 简洁命名与返回导航

- 模态框和功能卡片标题只使用功能本名，不用中点拼接公司名、代码、状态或额外解释。单位、日期等必要信息放在独立元数据或 info 提示中。
- 公司详情和报告面包屑只显示标准公司名称，代码保留在路由 slug；加载期间不先用代码占位。
- 页面返回统一使用 BackLink：同一 ArrowLeft 16px 图标、ghost 按钮、统一间距和焦点/悬停反馈；可见文字为“返回”，可访问名称说明返回目的地。通过 asChild 使用宿主路由链接，禁止各页面自行组合不同尺寸箭头。触摸设备命中高度至少44px。

## 受限宽度内容

- 表格和卡片必须适应容器宽度，不以固定最小宽度挤出页面。公司名、资料标题等短标签使用 OverflowText 单行省略，悬停或键盘聚焦时由浮层临时显示全文，不撑高原行。
- InfoLabel 与标签共同收缩，图标不收缩；图标与可见文字统一4px间距。
- 内容卡片按内容高度布局，不为来源链接或空状态保留大块固定高度。
