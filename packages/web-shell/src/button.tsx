...
// 跨 app 共享按钮：三端（content-factory / navigator / userportal）唯一的 Button 真相源。
// 语义真相源：根 design.md「间距与尺寸」（紧凑 32px / 表单主控件 36px）、「动效」
// （按下缩放 0.98，reduced-motion 关闭）与「无障碍与系统偏好」（coarse pointer ≥44px 命中区）。
// 各 app 的 `components/ui/button` 只做转出，禁止再分叉 size/variant 表。
...
// Button 必须支持 shadcn 式 `className` 覆盖，因此这里用 twMerge 消解同族冲突，
// 不能复用 index.tsx 里那个只做拼接的 shell `cn`。
